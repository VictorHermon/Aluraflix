...
class AuthenticationUserTestCase(APITestCase):
    ...
    def test_autenticacao_com_username_incorreto(self):
        """Teste que verifica a autenticação com um username incorreto"""
        user = authenticate(username='c3pp', password='123456')
        self.assertFalse((user is not None) and user.is_authenticated)

    def test_autenticacao_com_password_incorreto(self):
        """Teste que verifica a autenticação com um password incorreto"""
